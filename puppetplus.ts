...
export const loadUrl = async (url, log = false, howLongToWait = 0, page) => {
  try {
    await page.goto(url)
    ...
    console.log(
      `${url} failed! ${e.message} - retrying after ${howLongToWait}...`,
    )
...
const processUrl: (...args) => Promise<Card> = async (url, tab, setName) => {
  ...
  const [thirty, seven, one] = $(".info-list-container").find("dt").filter((
    i,
    el,
  ) => $(el).text().includes("average price"))
    .next("dd")
    .map((i, el) => $(el).text())
    .get()

  const rarity = $(
    "#tabContent-info > div > div.col-12.col-lg-6.mx-auto > div > div.info-list-container.col-12.col-md-8.col-lg-12.mx-auto.align-self-start > dl > dd:nth-child(2) > span",
...
export const getCardSetPrices = async (setName, tab, totalPages = 4) => {
  const baseUrl =
    `https://www.cardmarket.com/en/Pokemon/Products/Singles/${setName}`
  const urls: string[] = []
  const cards: Card[] = []

  for (let page = 1; page <= totalPages; page++) {
    const $ = await loadUrl(
      `${baseUrl}?sortBy=price_desc&site=${page}`,
      true,
      0,
      tab,
    )
  ...
  for (const url of urls) {
    ...
    while (Date.now() < end) {} // would be useful use of semicolon!
  }
